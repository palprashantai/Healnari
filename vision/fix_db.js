...
async function run() {
  ...
  // 1. Delete all payouts to reset available balance
  await supabase.from('payouts').delete().eq('doctor_id', doctorId);
  console.log('Deleted payouts for doctor');

  // 2. Update all payments to the current month so "Earnings This Month" displays correctly
  const now = new Date().toISOString();
  await supabase.from('payments').update({ created_at: now }).eq('doctor_id', doctorId);
  console.log('Updated payments created_at to', now);
}
