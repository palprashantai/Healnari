...
export const CRON_CATALOG: Record<string, CronMetadata> = {
  appointments_reminder_30min: {
    ...
    description:
      'Sweeps upcoming appointments within 30 minutes and reminds patients to complete device checks.',
    defaultSchedule: '*/5 * * * *',
  },
  appointments_queue_delay: {
    ...
    category: 'Appointments',
    description:
      'Projects waiting room start times and alerts patients if the doctor is running >15 minutes behind.',
    defaultSchedule: '*/5 * * * *',
  },
  appointments_unpaid_release: {
    ...
    category: 'Appointments',
    description:
      'Auto-cancels booking slots where payment was abandoned after 5 minutes, unlocking them for others.',
    defaultSchedule: '*/5 * * * *',
  },
  prescription_refill_reminders: {
    ...
    category: 'Patient',
    description:
      'Alerts patients with active prescriptions when 5 or fewer days of medication remain.',
    defaultSchedule: '0 9 * * *',
  },
  prescription_follow_up_reminders: {
    ...
    category: 'Patient',
    description:
      'Reminds patients when their recommended 2-week review window arrives after teleconsultation.',
  ...
  prescription_pending_lab_reminders: {
    ...
    category: 'Patient',
    description:
      'Reminds patients with doctor-requested lab tests older than 3 days who have not uploaded results.',
  ...
  cycle_period_prediction: {
    ...
    category: 'Patient',
    description:
      'Calculates cycle lengths and notifies women 2 days before predicted menstruation.',
    defaultSchedule: '0 7 * * *',
  },
  cycle_fertile_window: {
    ...
    category: 'Patient',
    description:
      'Alerts patients tracking fertility of their 5-day conception window and peak ovulation.',
  ...
  doctor_daily_agenda: {
    ...
    category: 'Doctor',
    description:
      'Sends daily schedule summaries of video consults and clinic visits to doctors.',
  ...
  doctor_stale_consultation_archival: {
    ...
    category: 'Doctor',
    description:
      'Auto-closes abandoned consultations left in waiting or in-progress overnight.',
    defaultSchedule: '0 2 * * *',
  },
  admin_daily_revenue_reconciliation: {
    ...
    category: 'Admin',
    description:
      'Aggregates 24-hour gross revenue, 15% platform commission, and doctor net payouts.',
    defaultSchedule: '0 0 * * *',
  },
  admin_doctor_kyc_escalation: {
    ...
    category: 'Admin',
    description:
      'Alerts administrators if doctor verification submissions have been pending >48 hours.',
  ...
  billing_automated_refunds: {
    ...
    category: 'Billing',
    description:
      'Sweeps cancelled consultations with active payments and triggers automatic Cashfree refunds.',
...
export class CronManagerService implements OnApplicationBootstrap {
  ...
  async restorePersistedConfigurations() {
    try {
        ...
        if (job) {
          ...
          if (config.expression) {
            try {
              ...
              this.logger.warn(
                `Failed to restore schedule for ${config.name}: ${err.message}`,
              );
            }
          }
      ...
      this.logger.log(
        `Successfully synced ${configs.length} cron configuration(s) from Supabase database.`,
      );
    } catch (err) {
      this.logger.warn(
        `Could not restore cron configurations from database: ${err.message}`,
      );
    }
  }
  ...
  async toggleCron(name: string, shouldRun?: boolean) {
    ...
    const currentRunning =
      job.running !== undefined ? Boolean(job.running) : newState;
    const meta = CRON_CATALOG[name] || this.inferJobMetadata(name);
    const cronTime = job.cronTime;
    const expression = cronTime?.source || meta.defaultSchedule;

    // Persist configuration in Supabase
    this.persistCronConfig(
      name,
      meta.displayName,
      meta.category,
      String(expression),
  ...
  async runCronNow(name: string) {
    ...
    try {
      ...
      const durationMs = Date.now() - startTime;
      this.logger.error(
        `Error running cron '${name}' on-demand: ${err.message}`,
      );
      ...
      throw new BadRequestException(
        `Failed to execute cron job '${name}': ${err.message}`,
      );
    }
  }
  ...
  async updateCronSchedule(name: string, expression: string) {
    ...
    try {
      const isCurrentlyRunning =
        job.running !== undefined ? Boolean(job.running) : true;
      ...
      this.logger.log(
        `Admin updated schedule for cron '${name}' to '${expression}'`,
      );

      const nextDate =
        typeof job.nextDate === 'function' ? job.nextDate() : null;
      const meta = CRON_CATALOG[name] || this.inferJobMetadata(name);
      const isRunning = job.running !== undefined ? Boolean(job.running) : true;
      ...
      throw new BadRequestException(
        `Invalid cron expression '${expression}': ${err.message}`,
      );
    }
  }
  ...
  private async persistCronConfig(
    ...
    try {
      await this.supabase.admin.from('cron_configurations').upsert(
        {
          name,
      ...
      this.logger.warn(
        `Could not save cron configuration for ${name} in Supabase: ${err.message}`,
      );
    }
  }
  ...
  async logExecution(log: {
    ...
    try {
      ...
      this.logger.warn(
        `Could not record execution log for ${log.job_name}: ${err.message}`,
      );
    }
  }
  ...
  private getJobOrThrow(name: string): any {
    try {
      ...
      throw new NotFoundException(
        `Cron job '${name}' was not found in active scheduler registry.`,
      );
    }
  }
}
