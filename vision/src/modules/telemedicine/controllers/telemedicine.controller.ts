...
export class TelemedicineController {
  ...
  @ApiOperation({ summary: "Doctor's video-consult queue (today + upcoming, video appointments only)" })
  @Get('queue')
  ...
  async getNotes(@CurrentUser() user: AuthUser, @Param('appointmentId') appointmentId: string) {
    const data = await this.telemedicineService.getNotes(user, appointmentId);
    return ResponseHelper.success(data, SUCCESS_MESSAGES.DATA_RETRIEVED);
  }

  @ApiOperation({ summary: 'Save a clinical note during/after a video session (doctor only)' })
  @ApiParam({ name: 'appointmentId' })
  @Post(':appointmentId/notes')
  async addNote(@CurrentUser() user: AuthUser, @Param('appointmentId') appointmentId: string, @Body() body: AddNoteDto) {
    const data = await this.telemedicineService.addNote(user, appointmentId, body.note);
