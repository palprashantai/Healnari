...
} from '@/modules/patients/services/fertility-prediction.util';
import {
  CreatePatientDto,
  InviteConnectionDto,
  JoinWaitlistDto,
...
export class PatientsService {
  ...
  private async assemble(profile: Profile, record: PatientRecord | null) {
    ...
    const doctorIds = [
      ...new Set(
        [
          ...
          ...notes.map((n) => n.doctor_id),
        ].filter(Boolean),
      ),
    ];
    let doctorById = new Map<
      string,
      {
        full_name: string;
    ...
    if (doctorIds.length) {
      const { data: doctors } = await this.supabase.admin
        .from('profiles')
        .select('id, full_name, specialty, registration_no')
        .in('id', doctorIds);
    ...
    return {
      ...
      prescriptions: prescriptions.map((p) => ({
        ...p,
        doctor_name:
          (p.doctor_id && doctorById.get(p.doctor_id)?.full_name) ||
          'Your Doctor',
        doctor_specialty:
          (p.doctor_id && doctorById.get(p.doctor_id)?.specialty) || null,
        doctor_registration_no:
          (p.doctor_id && doctorById.get(p.doctor_id)?.registration_no) || null,
      })),
      lab_reports: reportsRes.data || [],
      clinical_notes: notes.map((n) => ({
        ...n,
        doctor_name:
          (n.doctor_id && doctorById.get(n.doctor_id)?.full_name) ||
          'Your Doctor',
      })),
  ...
  async list(user: AuthUser) {
    ...
    data.forEach((row) => {
      (row.prescriptions || []).forEach((p: any) => {
        if (p.doctor_id) allDoctorIds.add(p.doctor_id);
      });
      (row.clinical_notes || []).forEach((n: any) => {
        if (n.doctor_id) allDoctorIds.add(n.doctor_id);
      });
    });
  ...
  async update(user: AuthUser, patientId: string, body: UpdatePatientDto) {
    ...
    if (body.bloodGroup !== undefined)
      recordPatch.blood_group = body.bloodGroup;
    if (body.heightCm !== undefined)
      recordPatch.height_cm = String(body.heightCm);
    if (body.weightKg !== undefined)
      recordPatch.weight_kg = String(body.weightKg);
    ...
    if (body.chronicConditions !== undefined)
      recordPatch.chronic_conditions = body.chronicConditions;
    if (body.lifeStageMode !== undefined)
      recordPatch.life_stage_mode = body.lifeStageMode;
    if (body.currencyPreference !== undefined)
      recordPatch.currency_preference = body.currencyPreference;
    if (body.lutealPhaseDays !== undefined)
      recordPatch.luteal_phase_days = body.lutealPhaseDays;
  ...
  async getFertilityPrediction(user: AuthUser) {
    ...
    const [{ data: logs }, { data: record }] = await Promise.all([
      this.supabase.admin
        .from('cycle_logs')
        .select('log_date, flow, bbt, lh_ratio, cervical_mucus')
        .eq('patient_id', user.id)
        .is('deleted_at', null)
        .order('log_date', { ascending: true }),
      this.supabase.admin
        .from('patient_records')
        .select('chronic_conditions, luteal_phase_days')
        .eq('patient_id', user.id)
        .is('deleted_at', null)
        .maybeSingle(),
    ]);

    const pcosFlag = (record?.chronic_conditions || []).some((c: string) =>
      /pcos|pcod/i.test(c),
    );
    const biomarkerLogs = (logs || [])
      .filter(
        (l: any) =>
          l.bbt != null || l.lh_ratio != null || l.cervical_mucus != null,
      )
      .map((l: any) => ({
        date: l.log_date,
  ...
  async quickFertilityEstimate(
    user: AuthUser,
    body: QuickFertilityEstimateDto,
  ) {
    if (user.profile.role !== ProfileRole.PATIENT)
      throw new ForbiddenException(ERROR_MESSAGES.FORBIDDEN);
    ...
    await this.supabase.admin.from('cycle_logs').upsert(rows, {
  ...
  async logCycle(user: AuthUser, date: string, body: LogCycleDto) {
    ...
    if (!log) {
      const { data: newLog } = await this.supabase.admin
        .from('cycle_logs')
        .insert({ patient_id: user.id, log_date: date, symptoms: [], ...body })
        .select()
        .is('deleted_at', null)
        .maybeSingle();
      return newLog;
  ...
  async inviteConnection(user: AuthUser, body: InviteConnectionDto) {
    ...
    const { data: existing } = await this.supabase.admin
      ...
      .maybeSingle();
    if (existing)
      throw new BadRequestException(ERROR_MESSAGES.CONNECTION_ALREADY_INVITED);
  ...
  private async findOwnConnection(user: AuthUser, id: string) {
    const { data: connection } = await this.supabase.admin
      .from('care_connections')
      .select()
      .eq('id', id)
      .eq('patient_id', user.id)
      .maybeSingle();
    if (!connection)
      throw new NotFoundException(ERROR_MESSAGES.CONNECTION_NOT_FOUND);
    return connection;
  }
  ...
  async updateConnectionPermissions(
    user: AuthUser,
    id: string,
    body: UpdateConnectionPermissionsDto,
  ) {
    if (user.profile.role !== ProfileRole.PATIENT)
      throw new ForbiddenException(ERROR_MESSAGES.FORBIDDEN);
  ...
  async addFavorite(user: AuthUser, doctorId: string) {
    ...
    const { data: existing } = await this.supabase.admin
      ...
      .maybeSingle();
    if (existing)
      throw new BadRequestException(ERROR_MESSAGES.FAVORITE_ALREADY_EXISTS);
  ...
  async leaveWaitlist(user: AuthUser, id: string) {
    if (user.profile.role !== ProfileRole.PATIENT)
      throw new ForbiddenException(ERROR_MESSAGES.FORBIDDEN);
    const { data: entry } = await this.supabase.admin
      .from('appointment_waitlist')
      .select()
      .eq('id', id)
      .eq('patient_id', user.id)
      .maybeSingle();
    if (!entry)
      throw new NotFoundException(ERROR_MESSAGES.WAITLIST_ENTRY_NOT_FOUND);
