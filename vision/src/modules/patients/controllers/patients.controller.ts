...
export class LogLifestyleDto {
  @ApiProperty({ description: 'Map of habit key -> completed', example: { sleep: true, water: false } })
  @IsObject()
...
export class UpdateConnectionPermissionsDto {
  @ApiProperty({ description: 'Map of permission key -> enabled', example: { cycleWindow: true, appointments: false, detailedRx: false } })
  @IsObject()
...
export class PatientsController {
  ...
  @ApiOperation({ summary: "Get the caller's own vitals (latest + previous reading per key)" })
  @Get('me/vitals')
  ...
  @ApiOperation({ summary: 'Log a new vitals reading (weight, bp, sugar, sleep, or hirsutism)' })
  @ApiParam({ name: 'key', enum: ['weight', 'bp', 'sugar', 'sleep', 'hirsutism'] })
  @Put('me/vitals/:key')
  async logVital(@CurrentUser() user: AuthUser, @Param('key') key: string, @Body() body: LogVitalDto) {
  ...
  @ApiOperation({ summary: 'Upsert the daily lifestyle checklist for a given date (YYYY-MM-DD)' })
  @ApiParam({ name: 'date' })
  @Put('me/lifestyle-logs/:date')
  async logLifestyle(@CurrentUser() user: AuthUser, @Param('date') date: string, @Body() body: LogLifestyleDto) {
  ...
  async inviteConnection(@CurrentUser() user: AuthUser, @Body() body: InviteConnectionDto) {
  ...
  @ApiOperation({ summary: 'Update what a care circle connection is allowed to see' })
  @ApiParam({ name: 'id' })
  @Put('me/care-connections/:id/permissions')
  async updateConnectionPermissions(@CurrentUser() user: AuthUser, @Param('id') id: string, @Body() body: UpdateConnectionPermissionsDto) {
    const data = await this.patientsService.updateConnectionPermissions(user, id, body);
    return ResponseHelper.success(data, SUCCESS_MESSAGES.CONNECTION_PERMISSIONS_UPDATED);
  }

  @ApiOperation({ summary: 'Remove a care circle connection' })
  @ApiParam({ name: 'id' })
  @Delete('me/care-connections/:id')
  async removeConnection(@CurrentUser() user: AuthUser, @Param('id') id: string) {
  ...
  @Post('me/favorites')
  async addFavorite(@CurrentUser() user: AuthUser, @Body() body: AddFavoriteDto) {
  ...
  async removeFavorite(@CurrentUser() user: AuthUser, @Param('doctorId') doctorId: string) {
  ...
  @ApiOperation({ summary: "Get the caller's waitlist entries, with computed queue position" })
  @Get('me/waitlist')
  ...
  @Post('me/waitlist')
  async joinWaitlist(@CurrentUser() user: AuthUser, @Body() body: JoinWaitlistDto) {
