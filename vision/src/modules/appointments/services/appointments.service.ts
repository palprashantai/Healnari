  ...
  private async withNames(appointments: any[]) {
    ...
    return appointments.map((a) => {
      ...
      const out = {
        ...
        fee: pricingLock?.patient_payable_amount ?? a.patient_payable_amount ?? a.fee ?? ((a.currency || 'INR') === 'USD' ? 29 : 799),
        base_fee_amount: pricingLock?.base_fee_amount ?? a.base_fee_amount ?? a.fee ?? ((a.currency || 'INR') === 'USD' ? 29 : 799),
        base_fee_currency: pricingLock?.base_fee_currency ?? a.base_fee_currency ?? a.currency ?? 'INR',
        patient_payable_amount: pricingLock?.patient_payable_amount ?? a.patient_payable_amount ?? a.fee ?? ((a.currency || 'INR') === 'USD' ? 29 : 799),
