...
import {
  ...
  CreateStaffDto,
  UpdateStaffDto,
} from '@/modules/staff/controllers/staff.controller';
...
export class StaffService {
  ...
  private async ownStaff(user: AuthUser, id: string) {
    const { data: staff } = await this.supabase.admin
      .from('staff_members')
      .select()
      .eq('id', id)
      .maybeSingle();
    if (!staff) throw new NotFoundException(ERROR_MESSAGES.STAFF_NOT_FOUND);
    if (staff.doctor_id !== user.id)
      throw new ForbiddenException(ERROR_MESSAGES.FORBIDDEN);
    return staff;
  }
  ...
  async updateLeave(user: AuthUser, id: string, status: string) {
    this.requireDoctor(user);
    const { data: leave } = await this.supabase.admin
      .from('leave_requests')
      .select()
      .eq('id', id)
      .maybeSingle();
    if (!leave) throw new NotFoundException(ERROR_MESSAGES.LEAVE_NOT_FOUND);
    if (leave.doctor_id !== user.id)
      throw new ForbiddenException(ERROR_MESSAGES.FORBIDDEN);
