...
export class CreatePrescriptionDto {
  ...
  @ApiProperty({
    required: false,
    description:
      'Base64 image or URL of handwritten/scanned prescription canvas',
  })
  @IsOptional()
  @IsString()
  ...
  @ApiProperty({
    ...
    description:
      'Every medicine written in this visit — all saved as one prescription (shared group_id), not as separate unrelated prescriptions.',
  })
  @IsArray()
  @ArrayMinSize(1)
...
export class RecordsController {
  ...
  @ApiOperation({
    summary: 'List prescriptions (patient: own; doctor: written by them)',
  })
  @Get('prescriptions')
  ...
  @ApiOperation({
    summary: 'Patient requests a refill on their own prescription',
  })
  @ApiParam({ name: 'id' })
  ...
  @ApiOperation({
    summary: 'Doctor approves or rejects a pending refill request',
  })
  @ApiParam({ name: 'id' })
  ...
  async handleRefill(
    ...
    const msg =
      body.action === 'approve'
        ? SUCCESS_MESSAGES.PRESCRIPTION_REFILL_APPROVED
        : SUCCESS_MESSAGES.PRESCRIPTION_REFILL_DENIED;
    return ResponseHelper.success(data, msg);
  }

  @ApiOperation({
    summary: 'List lab reports (patient: own; doctor: must pass ?patientId=)',
  })
  @Get('lab-reports')
  async getLabReports(
    @CurrentUser() user: AuthUser,
  ...
  @ApiOperation({
    summary:
      "Upload a lab report file (patient uploads their own; doctor may upload on a patient's behalf)",
  })
  ...
  @ApiOperation({
    summary: 'Get a short-lived signed URL to view/download a lab report file',
  })
  @ApiParam({ name: 'id' })
  ...
  @ApiOperation({
    summary: 'Doctor records review notes and marks a lab report Reviewed',
  })
  @ApiParam({ name: 'id' })
  ...
  @ApiOperation({
    summary: 'Patient deletes their own report before it has been reviewed',
  })
  @ApiParam({ name: 'id' })
  ...
  async requestLabReport(
    @CurrentUser() user: AuthUser,
    @Body() body: RequestLabReportDto,
  ) {
  ...
  @ApiOperation({
    summary:
      'List lab report requests (patient: own; doctor: ones they made, optionally ?patientId=)',
  })
  ...
  async cancelLabReportRequest(
    ...
    return ResponseHelper.success(
      data,
      SUCCESS_MESSAGES.LAB_REPORT_REQUEST_CANCELLED,
    );
  }

  @ApiOperation({
    summary:
      'Add a general clinical/chart note for a patient (doctor only, not tied to a specific appointment)',
  })
  @Post('notes')
  ...
  @ApiOperation({
    summary:
      "List a patient's records-vault documents (patient: own; doctor: any)",
  })
  ...
  @ApiOperation({
    summary:
      "Add a document to a patient's records vault (metadata only — no file storage yet)",
  })
  @Post('documents')
  async addDocument(
    @CurrentUser() user: AuthUser,
  ...
  @ApiOperation({
    summary: "List a patient's vaccinations (patient: own; doctor: any)",
  })
  ...
  @ApiOperation({
    summary: "List a patient's emergency contacts (patient: own; doctor: any)",
  })
  ...
  @ApiOperation({
    summary:
      'Get medicine or lab test catalog items (returns global + doctor custom items)',
  })
  @Get('catalog')
  async getCatalog(
    @CurrentUser() user: AuthUser,
    @Query('type') type?: 'medicine' | 'lab_test',
  ) {
  ...
  @ApiOperation({
    summary:
      'Add a new medicine or lab test to catalog (doctor custom or admin global)',
  })
  @Post('catalog')
  ...
  @ApiOperation({
    summary: 'Get clinical protocol bundles (global + doctor-custom)',
  })
  @Get('protocols')
