...
export function getProviderCapabilities(userOrProfile = null) {
  ...
  const navCategories = [
    ...
    },
    {
      title: 'Scheduling',
      items: [
        { name: isYogaExpert ? 'Session Queue' : 'Appointments', icon: 'fa-calendar-check', path: '/doctor-dashboard/appointments', end: false, color: '#6B46C1' },
        { name: 'My Schedule', icon: 'fa-clock', path: '/doctor-dashboard/schedule', end: false, color: '#6B46C1' },
        { name: `${def.clientLabel} Requests`, icon: 'fa-user-plus', path: '/doctor-dashboard/requests', end: false, color: '#6B46C1' },
      ],
    },
    {
      title: isPhysician ? 'Clinical Care' : 'Care & Deliverables',
      items: [
        { name: `${def.clientLabel}s & EMR`, icon: 'fa-users', path: '/doctor-dashboard/patients', end: false, color: '#6B46C1' },
        ...(def.canPrescribeDrugs ? [{ name: 'Prescriptions', icon: 'fa-file-prescription', path: '/doctor-dashboard/prescriptions', end: false, color: '#6B46C1' }] : []),
        ...(isNutritionist ? [{ name: 'Diet & Nutrition Plans', icon: 'fa-seedling', path: '/doctor-dashboard/prescriptions', end: false, color: '#6B46C1' }] : []),
        ...(isYogaExpert ? [{ name: 'Yoga & Movement Plans', icon: 'fa-om', path: '/doctor-dashboard/prescriptions', end: false, color: '#6B46C1' }] : []),
        ...(isMentalHealth ? [{ name: 'Session Plans & Notes', icon: 'fa-brain', path: '/doctor-dashboard/prescriptions', end: false, color: '#6B46C1' }] : []),
        { name: 'Telemedicine', icon: 'fa-video', path: '/doctor-dashboard/telemedicine', end: false, color: '#6B46C1' },
        ...(def.canReviewLabs ? [{ name: 'Lab & Reports', icon: 'fa-flask', path: '/doctor-dashboard/reports', end: false, color: '#6B46C1' }] : []),
      ],
    },
    {
      ...
      items: [
        ...
        { name: 'Earnings & Payouts', icon: 'fa-file-invoice-dollar', path: '/doctor-dashboard/billing', end: false, color: '#6B46C1' },
