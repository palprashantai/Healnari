              ...
              <div key={i} className="border border-slate-200 rounded-xl p-4 bg-slate-50">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">{m.schedule}</p>
                    <p className="text-xs text-slate-400">Duration: {m.duration}</p>
                  </div>
                  {m.refills > 0 && (
                    <span className="text-[10px] bg-emerald-50 text-emerald-700 border border-emerald-100 font-bold px-2 py-0.5 rounded-full">{m.refills} Refills Left</span>
                  )}
                </div>
              </div>
            ))}
            ...
            <div key={rx.id} className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-md transition-shadow">
              ...
              <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                <div className="flex items-center gap-3">
                  <div className={`w-11 h-11 rounded-xl flex items-center justify-center ${rx.status === 'Active' ? 'bg-aubergine-100 text-aubergine-700' : 'bg-slate-200 text-slate-500'}`}>
                ...
                <span className={`px-3 py-1 rounded-full text-xs font-bold border ${rx.status === 'Active' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-slate-100 text-slate-500 border-slate-200'}`}>
                  {rx.status}
                </span>
              </div>
                    ...
                    <div key={i} className="border border-slate-100 rounded-xl p-3.5 bg-slate-50">
                      <div className="flex items-start justify-between">
                        <div>
                          ...
                          <div className="text-xs text-slate-500 mt-0.5">{m.schedule}</div>
                          <div className="text-xs text-slate-400">{m.duration}</div>
                        </div>
                        {m.refills > 0 && <span className="text-[10px] bg-sky-50 text-sky-600 border border-sky-100 font-bold px-1.5 py-0.5 rounded">{m.refills}×</span>}
                      </div>
                    </div>
                  ))}
                </div>
